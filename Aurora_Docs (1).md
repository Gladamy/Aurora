# Aurora UI Library
**Version 6.4.0** — A minimalistic, production-quality UI library for Roblox executors.
...
| `element.SetRows(rows)` | Replace all rows at once |
...
| `element.Clear()` | Remove all rows |
| `element.GetRows()` | Returns current rows table |
...
-- Populate from live server data
for _, p in ipairs(game.Players:GetPlayers()) do
...
tbl.OnRowClicked:Connect(function(index, row)
    ...
    Aurora:Notify({ Title = "Selected", Message = row[1], Type = "Info", Duration = 2 })
end)

-- Live update a single cell (e.g. kill count)
tbl.SetCell(1, 2, 7)   -- row 1, column 2 = 7 kills
...
| Version | Changes |
|---|---|
| 6.4.0 | **Bug fix:** `TabContainer` `AbsoluteSize` reflow connection now stored in `windowConnections` — previously leaked after `Window:Destroy()` and errored when firing on a destroyed `ContentContainer` |
| 6.3.0 | `Tab.Name` field added · `GetRows()` returns shallow copy |
